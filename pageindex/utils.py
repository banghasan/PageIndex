...
# Ensure .env is loaded from project root even when running from other directories.
...
def count_tokens(text, model=None):
    ...
    try:
        ...
        logging.warning(
            "Tokenizer tidak ditemukan untuk model '%s'. Menggunakan fallback cl100k_base.",
            model,
        )
...
class ConfigLoader:
    ...
    def load(self, user_opt=None) -> config:
        ...
        env_dict = {}
        env_model = os.getenv("PAGEINDEX_MODEL") or os.getenv("MODEL")
        if env_model:
            env_dict["model"] = env_model
        merged = {**self._default_dict, **env_dict, **user_dict}
